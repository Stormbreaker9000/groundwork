...
This stage does not write ADRs or C4 diagrams. `.sdlc/design/adr/` and
`.sdlc/design/diagrams/` are declared dispatch slots owned by separate,
not-yet-built tickets (STO-100 and STO-101 respectively) — an empty or
absent `adr/`/`diagrams/` directory after this skill runs is expected, not a
bug. A deferred architecture decision is instead recorded as a `Q-` open
question in `assumptions.md` until STO-100 exists to turn it into an ADR.
