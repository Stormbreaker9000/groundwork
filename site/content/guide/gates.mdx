...
The two advisory linters use the same `2`, and otherwise always exit `0`.
...
ID, its file path, and the exact edit to make — move the edge onto the named
requirement's `traces_from`. You make that edit in the requirements stage, then
re-run the design stage.
