...
`FR-001`'s `traces_from: [CON-002, BR-002]` says this requirement exists
because of an offline constraint and a business rule, and you can follow both.
...
The acceptance criteria are Gherkin because they are meant to become tests. The
Definition of Done generator emits one gate per functional requirement that
asserts "all of that FR's acceptance-criteria scenarios pass", and it links the
requirement file rather than inlining the Gherkin — the requirement file stays
the authoritative, executable copy. That is also why `verification_method` on a
functional requirement is usually `test`.
