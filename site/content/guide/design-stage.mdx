...
The requirement set does not carry technology context — that is by
...
line, by judgment, at its gate. So nothing in `.sdlc/requirements/` says
"Tauri" or "Postgres" — because the pipeline is built to keep it that way, not
because a tool hard-fails on it.
